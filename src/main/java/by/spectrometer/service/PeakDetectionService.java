...
public class PeakDetectionService {
    ...
    public List<Peak> detectPeaks(double[] data, double baseline) {
        ...
        for (int i = peakWindow; i < data.length - peakWindow; i++) {
            ...
            if (isPeak) {
                Peak peak = new Peak(i, data[i]);
                calculatePeakProperties(peak, data, baseline);
                peaks.add(peak);
            }
        }
    ...
    private void calculatePeakProperties(Peak peak, double[] data, double baseline) {
        int pixel = peak.getPixel();
        ...
        peak.setLeftPixel(leftBoundary);
        peak.setRightPixel(rightBoundary);
        ...
        peak.setWidth(rightHalf - leftHalf);
        ...
        // Set base values
        peak.setLeftBase(baseline);
        peak.setRightBase(baseline);
    }
